...
def is_point_list(value):
    ...
    ret_val = True
    for x in value:
        if is_point(x) == False or len(x) != size:
            ret_val = False
    return ret_val
...
class Dataset(object):
    ...
    def __init__(self, dim, contents=None):
        ...
        """
        assert type(dim) == int, 'dim is not an int'
        assert dim > 0, 'dim is not greater than 0'
        ...
        if contents != None:
            assert is_point_list(contents), 'contents is not list of points'
            for x in contents:
                assert len(x) == dim, 'length of point is not dim'
                copy.append(x)
    ...
    def getPoint(self, i):
        ...
        """
        assert type(i) == int, 'i is not an int'
        assert (0 <= i <= self.getSize()-1) == True, 'i is not a valid position'
    ...
    def addPoint(self,point):
        ...
        to getDimension().
        """
        assert is_point(point) == True, 'point is not a valid point'
        assert len(point) == self.getDimension(), 'point dimension differs from set'
        pointadded = self._contents.append(point)
        return pointadded
